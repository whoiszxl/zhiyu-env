...
/* Use the active theme as a glass surface. The old --color-panel variable did
   not exist, so the palette accidentally rendered fully transparent. */
.command-palette{background:color-mix(in srgb,var(--color-bg-panel) 88%,transparent);color:var(--color-text-primary);backdrop-filter:blur(22px) saturate(1.08);-webkit-backdrop-filter:blur(22px) saturate(1.08)}
.command-search{background:color-mix(in srgb,var(--color-bg-elevated) 58%,transparent)}
.command-search input::placeholder{color:color-mix(in srgb,var(--color-text-primary) 68%,var(--color-bg-panel));opacity:1}
.command-group button.active{background:color-mix(in srgb,var(--color-selected) 82%,transparent)}
.command-group button small,.command-group button>b,.command-group button>em,.command-group>p{color:color-mix(in srgb,var(--color-text-primary) 66%,var(--color-bg-panel))}
.command-palette footer{background:color-mix(in srgb,var(--color-bg-muted) 78%,transparent);color:color-mix(in srgb,var(--color-text-primary) 68%,var(--color-bg-panel));backdrop-filter:blur(12px)}
:global(html[data-theme="light"]) .command-backdrop{background:rgba(35,40,36,.3);backdrop-filter:blur(8px) saturate(.88)}
:global(html[data-theme="light"]) .command-palette{background:color-mix(in srgb,var(--color-bg-panel) 92%,transparent);box-shadow:0 26px 80px rgba(25,31,27,.3)}
:global(html[data-theme="light"]) .command-group button strong{font-weight:650}
</style>
