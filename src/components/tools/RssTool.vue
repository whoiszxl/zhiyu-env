...
import { renderSafeRssHtml } from "../../utils/rssHtml";
...
const selectedContentHtml = computed(() => {
  ...
  return entry?.contentHtml
    ? renderSafeRssHtml(entry.contentHtml, entry.link)
    : "";
});
      ...
      <template v-else>
        <div class="rss-reader-toolbar">
          <div class="rss-reader-navigation">
            <button v-if="readingMode" type="button" :title="t('rss.backToList')" @click="closeReading">
              <span class="rss-toolbar-icon" aria-hidden="true">←</span>
              <span class="rss-toolbar-label">{{ t("rss.backToList") }}</span>
            </button>
            <button type="button" :disabled="!hasPreviousEntry" :title="t('rss.shortcutPrevious')" @click="navigateEntry(-1)">
              <span class="rss-toolbar-icon" aria-hidden="true">↑</span>
              <span class="rss-toolbar-label">{{ t("rss.previousArticle") }}</span>
            </button>
            <button type="button" :disabled="!hasNextEntry" :title="t('rss.shortcutNext')" @click="navigateEntry(1)">
              <span class="rss-toolbar-icon" aria-hidden="true">↓</span>
              <span class="rss-toolbar-label">{{ t("rss.nextArticle") }}</span>
            </button>
          </div>
          <div class="rss-reader-actions">
            <button
              type="button"
              :title="selectedEntry.isRead ? t('rss.markUnread') : t('rss.markRead')"
              @click="toggleRead(selectedEntry)"
            >
              <span class="rss-toolbar-icon" aria-hidden="true">{{ selectedEntry.isRead ? "○" : "●" }}</span>
              <span class="rss-toolbar-label">{{ selectedEntry.isRead ? t("rss.markUnread") : t("rss.markRead") }}</span>
            </button>
            <button type="button" :class="{ starred: selectedEntry.isStarred }" :title="t('rss.star')" @click="toggleStar(selectedEntry)">
              <span class="rss-toolbar-icon" aria-hidden="true">{{ selectedEntry.isStarred ? "★" : "☆" }}</span>
              <span class="rss-toolbar-label">{{ t("rss.star") }}</span>
            </button>
            <button
              type="button"
              class="rss-ai-trigger"
              :class="{ active: aiReaderOpen }"
              :title="t('rss.aiReader.button')"
              @click="aiReaderOpen = !aiReaderOpen"
            >
              <span class="rss-toolbar-icon" aria-hidden="true">✦</span>
              <span class="rss-toolbar-label">{{ t("rss.aiReader.button") }}</span>
            </button>
            <button class="primary" type="button" :disabled="!selectedEntry.link" :title="t('rss.openOriginal')" @click="openOriginal(selectedEntry)">
              <span class="rss-toolbar-icon" aria-hidden="true">↗</span>
              <span class="rss-toolbar-label">{{ t("rss.openOriginal") }}</span>
            </button>
          </div>
        </div>
        <div class="rss-reader-body">
          <div class="rss-reader-document">
            ...
            <div
              ...
              class="rss-content has-html"
              @click="handleContentClick"
              v-html="selectedContentHtml"
            ></div>
            <div v-else class="rss-content">{{ selectedContent }}</div>
...
.rss-reader{container-type:inline-size}.rss-reader-toolbar{flex-wrap:nowrap;justify-content:space-between;padding-block:8px}.rss-reader-navigation,.rss-reader-actions{display:flex;min-width:0;flex:0 0 auto;flex-wrap:nowrap;gap:6px}.rss-reader-actions{justify-content:flex-end;margin-left:auto}.rss-reader-toolbar button{flex:0 0 auto;white-space:nowrap}.rss-toolbar-icon{display:inline-grid;min-width:12px;place-items:center;font-size:10px;line-height:1}.rss-reader-toolbar .rss-ai-trigger{color:var(--color-accent)}.rss-reader-toolbar .rss-ai-trigger.active{border-color:var(--color-accent);background:color-mix(in srgb,var(--color-accent) 10%,var(--color-bg-panel))}.rss-page.reading-mode.ai-reader-open{grid-template-columns:minmax(0,1fr) minmax(360px,420px)}.rss-page.reading-mode.ai-reader-open .rss-reader{grid-column:1}.rss-page.reading-mode.ai-reader-open .rss-ai-panel{grid-column:2}@container(max-width:720px){.rss-reader-toolbar{gap:8px}.rss-reader-toolbar button{width:32px;min-height:32px;padding:0}.rss-toolbar-label{display:none}.rss-toolbar-icon{font-size:11px}}@media(max-width:1050px){.rss-page.reading-mode.ai-reader-open{grid-template-columns:minmax(0,1fr) minmax(330px,380px);padding-inline:14px}}@media(max-width:820px){.rss-page.reading-mode.ai-reader-open{position:relative;grid-template-columns:1fr}.rss-page.reading-mode.ai-reader-open .rss-ai-panel{position:absolute;z-index:20;inset:0 0 0 auto;width:min(420px,100%);box-shadow:-18px 0 50px rgba(0,0,0,.2)}}
.rss-content.has-html{white-space:normal}.rss-content.has-html :deep(p){margin:0 0 1.35em}.rss-content.has-html :deep(h1),.rss-content.has-html :deep(h2),.rss-content.has-html :deep(h3),.rss-content.has-html :deep(h4){margin:1.65em 0 .7em;color:var(--color-text-primary);line-height:1.35}.rss-content.has-html :deep(h1){font-size:1.7em}.rss-content.has-html :deep(h2){font-size:1.45em}.rss-content.has-html :deep(h3){font-size:1.25em}.rss-content.has-html :deep(h4){font-size:1.1em}.rss-content.has-html :deep(img){display:block;max-width:100%;height:auto;margin:22px auto;border:1px solid var(--color-border);background:var(--color-bg-muted);object-fit:contain}.rss-content.has-html :deep(figure){margin:24px 0}.rss-content.has-html :deep(figcaption){margin-top:-14px;color:var(--color-text-muted);font-size:.78em;text-align:center}.rss-content.has-html :deep(a){color:var(--color-accent);text-decoration:none}.rss-content.has-html :deep(a:hover){text-decoration:underline}.rss-content.has-html :deep(ul),.rss-content.has-html :deep(ol){margin:0 0 1.35em;padding-left:1.8em}.rss-content.has-html :deep(li){margin:.35em 0}.rss-content.has-html :deep(blockquote){margin:1.4em 0;padding:.7em 1em;border-left:3px solid var(--color-accent);background:var(--color-bg-muted)}.rss-content.has-html :deep(pre){max-width:100%;overflow:auto;padding:14px;border:1px solid var(--color-border);background:var(--terminal-bg);color:var(--terminal-fg)}.rss-content.has-html :deep(code){font-family:"SFMono-Regular",Consolas,monospace}.rss-content.has-html :deep(:not(pre)>code){padding:2px 4px;background:var(--color-bg-muted)}.rss-content.has-html :deep(table){display:block;max-width:100%;overflow:auto;border-collapse:collapse}.rss-content.has-html :deep(th),.rss-content.has-html :deep(td){padding:7px 9px;border:1px solid var(--color-border);text-align:left}.rss-content.has-html :deep(hr){margin:2em 0;border:0;border-top:1px solid var(--color-border)}
</style>
