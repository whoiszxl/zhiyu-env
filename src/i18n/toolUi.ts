...
const english = new Map<string, string>([
  ...
  ["清空", "Clear"],
  ["保存请求", "Save Request"],
  ["请求集合", "Request Collection"],
  ["搜索请求…", "Search requests…"],
  ["没有已保存的请求", "No saved requests"],
  ["填写请求后点击顶部“保存请求”", "Fill in a request, then select “Save Request”"],
  ["仅保存在本机", "Stored locally"],
  ["请求已保存", "Request saved"],
  ["本地域名", "Local Domains"],
  ["使用 .localhost 域名访问本地服务，由 Caddy 提供反向代理与 HTTPS", "Access local services with .localhost domains through Caddy reverse proxy and HTTPS"],
  ["网关运行中", "Gateway running"],
  ["网关未运行", "Gateway stopped"],
  ["应用路由", "Apply Routes"],
  ["应用中", "Applying"],
  ["恢复原配置", "Restore Original Config"],
  ["添加本地域名", "Add Local Domain"],
  ["域名", "Domain"],
  ["目标地址", "Target"],
  ["添加", "Add"],
  ["路由列表", "Routes"],
  ["打开 ↗", "Open ↗"],
  ["还没有路由", "No routes yet"],
  ["添加后点击“应用路由”，智屿会生成 Caddy 配置。", "Add a route, then apply it to generate the Caddy configuration."],
  ["无需修改 hosts", "No hosts File Changes"],
  ["*.localhost 会由浏览器解析到本机，不污染系统配置。", "*.localhost resolves locally without changing system configuration."],
  ["HTTPS 说明", "About HTTPS"],
  ["HTTPS 使用 Caddy 内部证书。首次使用时，系统可能提示需要信任本地 CA。", "HTTPS uses Caddy's internal CA. Your system may request trust on first use."],
  ["固定网关端口", "Fixed Gateway Ports"],
  ["HTTP 使用 8082，HTTPS 使用 8443，避免占用需要管理员权限的 80/443。", "HTTP uses 8082 and HTTPS uses 8443 to avoid privileged ports 80 and 443."],
  ["测试数据生成器", "Test Data Generator"],
  ["在本机生成 JSON、CSV 与 SQL 测试数据，不上传任何字段配置", "Generate JSON, CSV, and SQL test data locally without uploading schemas"],
  ["导出文件", "Export File"],
  ["生成数据", "Generate Data"],
  ["字段结构", "Schema"],
  ["添加字段", "Add Field"],
  ...
  ["表结构", "Table Schema"],
  ["代码模型", "Code Model"],
  ["开发查询模板", "Development Query Templates"],
  ["只生成查询，不会自动执行", "Queries are generated but never executed automatically"],
  ["放入格式化器", "Open in Formatter"],
  ["环境模板", "Environment Templates"],
  ["从经过整理的本地模板快速创建项目工作区，不下载脚本、不自动执行命令", "Create project workspaces from curated local templates without downloading or executing scripts"],
  ["打开项目工作区 ↗", "Open Project Workspace ↗"],
  ["搜索技术栈或服务…", "Search stacks or services…"],
  ["复制 .env 示例", "Copy .env Example"],
  ["使用模板", "Use Template"],
  ["模板边界", "Template Safety"],
  ["模板只会创建智屿项目配置，记录所需运行时、服务与环境变量名称。", "Templates only create Zhiyu project configuration with runtimes, services, and environment variable names."],
  ["不会写入业务源码，不会保存密码，也不会自动执行 npm、Maven 或 Shell 脚本。", "Templates never write source code, store passwords, or run npm, Maven, or shell scripts."],
  ["关闭", "Close"],
  ["✦ AI 助手", "✦ AI Assistant"],
  ["✦ AI 分析", "✦ AI Analyze"],
  ["✦ AI 诊断", "✦ AI Diagnose"],
  ["✦ AI 配置", "✦ AI Config"],
  ["✦ AI 生成", "✦ AI Generate"],
  ["✦ AI 批量生成", "✦ AI Batch Generate"],
  ["✦ AI 设计消息", "✦ AI Message Design"],
  ["✦ AI 命令建议", "✦ AI Command Advice"],
  ["生成", "Generate"],
  ["重新生成", "Regenerate"],
  ["应用到编辑器", "Apply to Editor"],
  ["打开 AI 设置", "Open AI Settings"],
  ["AI 仅生成建议，不会自动执行 · ⌘/Ctrl + Enter", "AI only generates suggestions and never runs them · ⌘/Ctrl + Enter"],
  ["生成结果会显示在这里", "Generated output will appear here"],
  ["正在连接模型…", "Connecting to model…"],
  ["请先配置并启用模型 API。", "Configure and enable a model API first."],
  ["生成 SQL", "Generate SQL"],
  ["分析与优化", "Analyze & Optimize"],
  ["修复错误", "Fix Error"],
  ["生成命令", "Generate Command"],
  ["批量生成", "Batch Generate"],
  ["设计消息", "Design Message"],
  ["AI 设计助手", "AI Design Assistant"],
  ["AI Mock API 生成器", "AI Mock API Generator"],
  ["确认 AI 生成的接口", "Review AI-Generated Routes"],
  ["冲突接口不会覆盖现有配置", "Conflicting routes will not overwrite existing configuration"],
  ["与现有接口冲突", "Conflicts with an existing route"],
  ["可导入", "Ready to import"],
  ["导入后会自动保存；服务运行中也会立即更新路由。", "Routes are saved automatically and applied to a running server immediately."],
  ["导入所选接口", "Import Selected Routes"],
  ["生成 Exchange、Queue、Routing Key 与消息结构建议", "Generate Exchange, Queue, Routing Key, and message contract suggestions"],
  ["根据业务事件生成 Topic、Key、分区数与 Payload", "Generate a Topic, Key, partition count, and Payload from a business event"],
  ["根据业务事件生成 Subject、订阅模式与 Payload", "Generate a Subject, subscription pattern, and Payload from a business event"],
  ["描述业务模块、接口、字段，以及希望覆盖的成功或异常场景", "Describe the module, routes, fields, and success or error scenarios to cover"],
  ["描述希望查询、聚合或修改的文档", "Describe the documents to query, aggregate, or modify"],
  ["分析当前命令、聚合管道、索引或最近错误", "Analyze the current command, aggregation pipeline, indexes, or latest error"],
  ["描述业务事件、生产者、消费者和期望的消息字段", "Describe the business event, producer, consumer, and expected message fields"],
  ["分析与建议", "Analysis & Advice"],
  ["诊断日志", "Diagnose Logs"],
  ["生成配置", "Generate Config"],
  ["生成请求", "Generate Request"],
  ["生成 Cron", "Generate Cron"],
  ["生成正则", "Generate Regex"],
  ["命令建议", "Command Advice"],
  ["取消", "Cancel"],
  ["选择", "Choose"],
